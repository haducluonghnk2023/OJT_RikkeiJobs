...
const changeStatus = async (record) => {
  ...
  try {
    ...
    const errorMessage = error.response ? extractErrorMessage(error) : (error.message || "Có lỗi xảy ra khi thay đổi trạng thái");
...
const changeRole = async ({ key, record }) => {
  try {
    ...
    // Lấy message từ backend
    const errorMessage = error.response ? extractErrorMessage(error) : (error.message || "Có lỗi xảy ra khi thay đổi vai trò");
    message.error(errorMessage);
  }
};
...
const lockUser = async (userId) => {
  try {
    ...
    // Lấy message từ backend
    const errorMessage = error.response ? extractErrorMessage(error) : (error.message || "Có lỗi xảy ra khi khóa tài khoản");
    message.error(errorMessage);
  }
};
