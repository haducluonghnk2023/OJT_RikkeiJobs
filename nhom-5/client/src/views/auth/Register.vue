...
const isUsernameAvailable = computed(() => {
  ...
  return !store.getters["auth/getUserByUsername"](formState.userName);
});

const isEmailAvailable = computed(() => {
  return !store.getters["auth/getUserByEmail"](formState.email);
});
...
const onFinish = async () => {
  try {
    ...
    // Gửi password plain text, server sẽ encode
    const userData = {
      email: formState.email,
      userName: formState.userName,
      password: formState.password, // Gửi password plain text, server sẽ encode
    };
    ...
    if (error.response) {
      switch (error.response.status) {
        case 400:
          ...
          break;
        case 409:
          message.error("Email hoặc tên đăng nhập đã tồn tại trong hệ thống");
          break;
        default:
