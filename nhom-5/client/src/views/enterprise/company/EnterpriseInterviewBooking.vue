...
const interviewBookingByEnterpriseId = computed(() => {
  const enterpriseIdNum = Number(id); // Convert route param to Number
  return store.state.interviewBooking.interviewBookings.filter((i) => {
    // Handle both String and Number enterpriseId
    const bookingEnterpriseId = typeof i.enterpriseId === 'string' 
      ? Number(i.enterpriseId) 
      : i.enterpriseId;
    return bookingEnterpriseId === enterpriseIdNum;
  });
});
