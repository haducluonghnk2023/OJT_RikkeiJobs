...
// Helper function to convert array to string (join by newlines)
...
watch(
  ...
  (newValue) => {
    if (newValue && props.edittedJob) {
      ...
      formData.description = arrayToString(props.edittedJob.description);
      formData.rank = Array.isArray(props.edittedJob.rank) 
        ? props.edittedJob.rank 
        : (props.edittedJob.rank ? [props.edittedJob.rank] : []);
      ...
      formData.benefitsDescription = arrayToString(props.edittedJob.benefitsDescription);
      formData.workingTime = props.edittedJob.workingTime || "";
      formData.required = arrayToString(props.edittedJob.required);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Helper function to convert string to array (split by newlines)
    ...
    const submitData = {
      ...formData,
      enterpriseId: Number(id), // Convert to Number (Long) to match Java Entity
      flight: "pending",
      ...
      deadline: formData.deadline
        ...
        : null,
      // Convert String to Array for fields that are List<String> in Java Entity
      description: stringToArray(formData.description),
      benefitsDescription: stringToArray(formData.benefitsDescription),
      required: stringToArray(formData.required),
    };
