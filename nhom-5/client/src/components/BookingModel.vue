...
const handleUp = async () => {
  ...
  // Helper function to find jobId from job title
  const findJobIdByName = (jobTitle) => {
    const foundJob = jobs.value.find((j) => j.title === jobTitle);
    return foundJob ? foundJob.id : null;
  };

  const formattedData = {
    ...formData,
    jobId: findJobIdByName(formData.job), // Convert job title to jobId
    enterpriseId: Number(formData.enterpriseId), // Ensure it's a Number
...
const filterJob = computed(() => {
  const enterpriseIdNum = Number(formData.enterpriseId);
  return jobs.value.filter((j) => {
    // Handle both String and Number enterpriseId
    const jobEnterpriseId = typeof j.enterpriseId === 'string' 
      ? Number(j.enterpriseId) 
      : j.enterpriseId;
    return jobEnterpriseId === enterpriseIdNum;
  });
});
